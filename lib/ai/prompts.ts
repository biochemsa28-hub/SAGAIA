...

{
  ...
  "production_notes": {
    ...
    "hueco": "EL HUECO de la ley 9, declarado en una frase: qué dato lateral queda SIN cerrar y en qué escenas se siembra. Ej: 'el sobre sin abrir que ella guarda en el delantal — se ve en las escenas 2 y 5, nunca se dice qué contiene'. OBLIGATORIO en todo formato: un guion sin hueco declarado se regenera",
...
3. LA PRUEBA DEL CIEGO (formatos hablados; en mudo la prueba es al revés: se entiende TODO sin audio): un oyente SIN pantalla tiene que poder contar la historia solo con el audio. Medido en un video muerto: «Fue un beso, solo eso. No fue solo eso. Era pasado. ¿Qué es eso?» — cinco "eso" y cero historia. PROHIBIDO "eso/esto/lo/aquello" sin su sustantivo en la MISMA línea: en telenovela se nombra la cosa y la persona («¿Una carta de Emilio? ¿Para ti?», «Besaste a mi hermana en mi boda»). Cada línea aporta UN dato nuevo y concreto que la anterior no tenía; una línea que solo reacciona («no puede ser», «¿qué?») se gana su lugar únicamente después de un dato fuerte. Y LA PRUEBA ESPEJO — EL SORDO: la mayoría del público de Facebook mira EN SILENCIO; los image_prompt y physical_action leídos en orden, SIN las líneas, también tienen que contar quién hizo qué a quién y dónde está el giro. Si la trama vive solo en el audio, el guion falla igual.
4. MANOS, NO POSES (todo formato): en cada escena el que habla (o el sujeto, si es mudo) HACE algo con las manos y un objeto mientras habla — arrebata la carta, la rompe, sirve la copa, empaca la maleta. Prohibido "de pie", "mirando", "con los brazos cruzados" como única acción: eso es un catálogo de modelos, no una escena. La cámara puede cortar al OBJETO que las manos manipulan — cada línea con su imagen nueva.
5. DIÁLOGO QUE LLENA, CON PRESUPUESTO DURO (solo formatos hablados; escena/mudo exento): TODAS las escenas tienen línea — máximo UNA muda, solo si el gesto del pico lo exige. EL PRESUPUESTO ES ARITMÉTICA, NO ESTILO: la voz corre a ~2.5 palabras por segundo, así que un video de 25s aguanta MÁXIMO ~70 palabras en total (30s → ~85). Ni una más: cada palabra de más desborda el contenedor y el montaje la paga en segundos. Repartilas ASIMÉTRICAS, como pelea la gente real: el que ataca lleva la línea larga (10-18 palabras, con el dato), el otro contesta CORTO (2-8: «Dos meses.», «Eso no fue así.», «Bájale.») — réplica y contrarréplica entre escenas, nunca dos monólogos. Medido en un video muerto: 26 palabras en 21s se sintió vacío; el techo evita el error inverso. En terror/suspenso el silencio es un arma y esta ley cede.
...
7. ASÍ DISCUTE LA GENTE DE VERDAD: la pelea se escribe como discuten las familias reales sobre ESTE tema — con los detalles domésticos que solo esa casa tiene: cifras («son DOS meses, no uno»), terceros con nombre («doña Mary ya vino a tocar ayer»), hechos concretos que se echan en cara («te gastaste el aguinaldo en la moto y yo pagando la luz»). LA PRUEBA: si una línea sirve tal cual para CUALQUIER otra pelea, está mal escrita — PROHIBIDAS las frases de tráiler («esto no arregla nada», «¿cómo vamos a seguir así?», «no puedo más», «ya no sé quién eres»). Medido en un video muerto: seis frases genéricas seguidas y el espectador dijo "palabras sin sentido". El vocativo (el nombre del otro) va máximo en UNA de cada tres líneas — la gente no repite tu nombre en cada frase. Ninguna línea se repite ni se repite con variación. El espectador tiene que pensar «así discutimos en mi casa». Y LA PRIMERA LÍNEA DEL VIDEO lleva una CIFRA o un NOMBRE PROPIO («Son dos meses de renta, Agustín», «Doña Mary vino a tocar otra vez») — lo concreto en el arranque es lo que la gente le reenvía a alguien.
8. PUESTA EN ESCENA CONTINUA: los cuerpos NO se teletransportan. Cada physical_action arranca EXACTAMENTE de la posición donde terminó la escena anterior: si estaban abrazados, la siguiente escena muestra el gesto de soltarse antes de verlos separados; si él está junto a la ventana, no aparece de pronto tocándole la cara — CAMINA. Medido: abrazo por detrás → separados frente a frente → mano en la cara → beso, sin un solo movimiento entre medio. Y el CIERRE es la CONSECUENCIA de la discusión (el portazo, el silencio, el objeto sobre la mesa, la maleta): una reconciliación o un beso SOLO si la premisa lo pide — una pelea por dinero no termina en beso porque sí.
9. EL HUECO (todo formato): el cierre RESUELVE la pregunta central del video — pero deja UN dato lateral concreto SIN cerrar, mencionado o mostrado al menos dos veces y jamás explicado: quién tocó la puerta y nunca se dijo quién era, la segunda nuez en la bandeja, el sobre que ella guardó sin abrir. NO es el cliffhanger de la parte 2 (ese ya existe): es el detalle que hace que alguien escriba en comentarios «¿y quién era el de la puerta?». Un video que cierra todo perfecto no deja nada que preguntar — medido: ocho videos con cero comentarios. El hueco se siembra, no se subraya. DECLARALO en production_notes.hueco (qué dato y en qué escenas vive): sin hueco declarado el guion se regenera.
10. EL GIRO ENTRE EL SEGUNDO 4 Y EL 7 (todo formato): la escena 1 dura como máximo ~6 segundos hablados — el VUELCO (la revelación, el objeto que cambia todo, la escena 2) tiene que estar EN PANTALLA entre el segundo 4 y el 7. Es la variable con la correlación más alta con la distribución medida en los videos reales de esta casa (+0.84 tiempo de reproducción ↔ alcance; el umbral vivo/muerto está en 6s de tiempo promedio). Una escena 1 que se estira más allá del segundo 7 es donde el espectador se va.
