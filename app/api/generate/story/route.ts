  ...
  try {
    ...
    if (result.data?.scenes?.length && !esEscena) {
      ...
      const defectosDe = (scenes: EscenaMin[]) => {
        ...
        // 17) GUION RALO. Drama hablado con aire muerto: medido en video
        //     terminado, 26 palabras en 21s (1.2 pal/s) con tramos mudos de 6s
        //     — "siento que no dice nada". Un drama/telenovela llena: mínimo
        //     ~1.6 pal/s del total. Terror/suspenso quedan exentos (ahí el
        //     silencio es un arma) y el formato escena es mudo por diseño.
        const esDrama = !esEscenaFmt && /drama|novela|traici|romance|amor|familia|pareja|celos|suegr/i.test(`${parsed.data.niche ?? ""} ${parsed.data.tone ?? ""} ${parsed.data.topic}`) && !/terror|horror|suspenso|thriller|miedo/i.test(`${parsed.data.niche ?? ""} ${parsed.data.tone ?? ""}`);
