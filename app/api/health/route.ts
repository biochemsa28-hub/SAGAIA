...
export async function GET(req: Request) {
  ...
  const gasto = {
    ...
    encadenado_real: process.env.CHAIN_REAL ?? "default(on)",
    fundido_empalmes: process.env.XFADE ?? "default(on)",
    efectos_montaje: process.env.EFECTOS ?? "default(on)",
    musica_auto: process.env.AUTO_MUSIC ?? "default(off)",
    hoja_2x2: process.env.CHARACTER_BIBLE ?? "default(off)",
